      ...
      <div className="grid grid-cols-1 xl:grid-cols-3 gap-4">
        <div className="xl:col-span-2 space-y-3">
           <motion.div 
             ...
             className="relative p-4 rounded-3xl overflow-hidden border border-white/5 bg-gradient-to-br from-secondary/10 to-transparent group"
           >
              ...
              <h1 className="text-xl font-black mb-4 tracking-tighter">مرحباً بك في <span className="text-secondary">Sumou ERP</span></h1>
              <p className="text-sidebar-text text-lg max-w-xl leading-relaxed">أهلاً بك مجدداً. النظام يراقب الآن كافة العمليات والوثائق لضمان استمرارية عمل مؤسستك بكفاءة عالية.</p>
              
              <div className="mt-10 flex gap-4">
                 <div className="px-3 py-2 bg-secondary text-primary rounded-2xl font-black text-sm flex items-center gap-2">
                    <ShieldCheck size={18} /> النظام مؤمن بالكامل
                 </div>
                 <div className="px-3 py-2 bg-white/5 border border-white/10 rounded-2xl font-bold text-sm text-sidebar-text flex items-center gap-2">
           ...
           {quickActions.map((action, idx) => (
             <Link key={idx} href={action.href}>
                <motion.div 
                  ...
                  className="h-full p-4 rounded-2xl bg-white/[0.02] border border-white/5 flex flex-col items-center justify-center text-center gap-4 hover:bg-white/[0.05] hover:border-secondary/20 transition-all cursor-pointer group"
                >
                   <div className={`w-14 h-10 rounded-2xl bg-${action.color}/10 flex items-center justify-center text-${action.color} group-hover:scale-110 transition-transform`}>
                      <action.icon size={18} />
                   </div>
