...
function CustomViewContent() {
  ...
  if (error) {
    return (
      <div className="flex flex-col items-center justify-center p-20 glass rounded-3xl text-center space-y-3">
  ...
  if (!pageInfo) {
    return (
      <div className="p-20 text-center space-y-3">
        <div className="w-16 h-12 border-4 border-secondary/20 border-t-secondary rounded-full animate-spin mx-auto"></div>
      ...
      <div className="glass p-4 rounded-3xl border border-white/5 bg-white/[0.01]">
         {data.length === 0 && !dataLoading ? (
           <div className="py-20 text-center border-2 border-dashed border-white/5 rounded-3xl">
              <Database size={18} className="mx-auto text-white/10 mb-4" />
              <p className="text-sidebar-text font-medium">هذا القسم فارغ حالياً.</p>
           </div>
         ) : (
           <div className="grid grid-cols-1 md:grid-cols-1 md:grid-cols-2 gap-4">
              {data.map((item) => (
                <div key={item.id} className="p-5 rounded-3xl bg-white/[0.01] border border-white/5 hover:border-secondary/20 transition-all group">
...
<button onClick={() => { if(window.confirm('هل أنت متأكد من الحذف؟ لا يمكن التراجع عن هذا الإجراء.')) removeItem(item.id!); }} className="p-3 text-rose-500 hover:bg-rose-500/10 rounded-xl opacity-0 group-hover:opacity-100 transition-all">
                         <Trash2 size={18} />
                      </button>
</div>
                   </div>
      ...
      <Modal isOpen={isModalOpen} onClose={() => setIsModalOpen(false)} title={`إضافة بيانات - ${pageInfo.label}`}>
         <form onSubmit={handleSubmit} className="space-y-3">
            <div className="space-y-2">
               ...
               <input required value={formData.name} onChange={e => setFormData({...formData, name: e.target.value})} className="w-full bg-white/5 border border-white/10 rounded-2xl py-2 px-3 outline-none focus:ring-2 focus:ring-secondary/20" />
            </div>
            <div className="space-y-2">
               <label className="text-xs font-black text-sidebar-text uppercase px-2">التفاصيل</label>
               <textarea required value={formData.details} onChange={e => setFormData({...formData, details: e.target.value})} className="w-full bg-white/5 border border-white/10 rounded-2xl py-2 px-3 outline-none h-32 focus:ring-2 focus:ring-secondary/20" />
            </div>
            <div className="space-y-2">
               <label className="text-xs font-black text-sidebar-text uppercase px-2">التاريخ</label>
               <input type="date" required value={formData.date} onChange={e => setFormData({...formData, date: e.target.value})} className="w-full bg-white/5 border border-white/10 rounded-2xl py-2 px-3 outline-none focus:ring-2 focus:ring-secondary/20" />
            </div>
            <div className="pt-6 flex gap-4">
               <button type="submit" className="flex-1 bg-secondary text-primary py-2 rounded-2xl font-black">حفظ البيانات</button>
               <button type="button" onClick={() => setIsModalOpen(false)} className="px-8 py-2 bg-white/5 rounded-2xl font-black">إلغاء</button>
            </div>
         </form>
      </Modal>
    </div>
