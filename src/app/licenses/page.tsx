  ...
  return (
    <div className="space-y-10 font-rubik">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
        <div>
          <h1 className="text-4xl font-black mb-2 tracking-tight text-white">التراخيص والسجلات</h1>
          <p className="text-sidebar-text font-medium text-lg">إدارة التراخيص الحكومية والسجلات التجارية لضمان الالتزام بكافة الأنظمة.</p>
        </div>
        <button 
      ...
      <div className="glass p-10 rounded-[4rem] border border-white/5 bg-white/[0.01]">
         <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {licenses.map((lic) => (
              <div key={lic.id} className="p-10 rounded-[3.5rem] bg-white/[0.02] border border-white/5 hover:border-secondary/20 transition-all group relative overflow-hidden">
                 <div className="flex items-center gap-6 mb-8">
                    <div className="w-16 h-16 rounded-[1.8rem] bg-secondary/10 flex items-center justify-center text-secondary">
                       <ShieldCheck size={32} />
                    </div>
                    <div>
                       <h4 className="font-black text-xl text-white">{lic.name}</h4>
                 ...
                 <button onClick={() => removeItem(lic.id!)} className="absolute top-8 left-8 p-3 text-rose-500 opacity-0 group-hover:opacity-100 transition-all hover:bg-rose-500/10 rounded-xl">
                    <Trash2 size={20} />
                 </button>
              </div>
            ))}
         ...
         {licenses.length === 0 && !loading && (
           <div className="py-24 text-center border-2 border-dashed border-white/5 rounded-[4rem]">
              ...
              <p className="text-sidebar-text text-xl font-black">لا توجد تراخيص مسجلة حالياً.</p>
           </div>
         )}
      </div>
        ...
        <div className="p-2">
           ...
           <div className="flex items-center justify-between mb-16 border-b border-white/5 pb-10">
              <div className="flex items-center gap-10 text-right">
                 ...
                 <div>
                    <h2 className="text-5xl font-black text-white tracking-tighter">إضافة ترخيص جديد</h2>
                    <p className="text-white/40 font-bold mt-3 text-xl">يرجى تعبئة كافة الحقول المطلوبة بدقة لضمان صحة بيانات الترخيص.</p>
                 </div>
              </div>
           </div>
           ...
           <form onSubmit={handleSubmit} className="space-y-14">
              <div className="grid grid-cols-2 gap-x-16 gap-y-12">
                 ...
                 <div className="space-y-5 text-right">
                    <label className="text-sm font-black text-white/20 uppercase tracking-[0.2em] px-6">اسم الترخيص</label>
                    <div className="relative group">
                       ...
                       />
                       <Building className="absolute left-10 top-1/2 -translate-y-1/2 text-white/5" size={28} />
                    </div>
                 </div>
                 <div className="space-y-5 text-right">
                    <label className="text-sm font-black text-white/20 uppercase tracking-[0.2em] px-6">رقم الترخيص</label>
                    <div className="relative group">
                       <input 
                 ...
                 {/* Row 2 */}
                 <div className="space-y-5 text-right">
                    <label className="text-sm font-black text-white/20 uppercase tracking-[0.2em] px-6">التصنيف</label>
                    <CustomSelect 
                 ...
                 <div className="space-y-5 text-right">
                    <label className="text-sm font-black text-white/20 uppercase tracking-[0.2em] px-6">النوع الإضافي</label>
                    <div className="relative group">
                       <input 
                         ...
                         className="w-full bg-white/[0.01] border border-white/10 rounded-[2.5rem] py-7 px-12 text-lg outline-none focus:ring-8 focus:ring-secondary/5 focus:border-secondary/40 transition-all text-right placeholder:text-white/5" 
                       />
                       <Tag className="absolute left-10 top-1/2 -translate-y-1/2 text-white/5" size={28} />
                    </div>
                 </div>
              ...
              {/* Upload Section */}
              <div className="space-y-5 text-right">
                 <label className="text-sm font-black text-white/20 uppercase tracking-[0.2em] px-6">صورة الترخيص</label>
              ...
              <div className="pt-16 flex gap-10">
                 <button type="submit" className="flex-[2] bg-secondary text-primary py-8 rounded-[3rem] font-black text-2xl shadow-2xl shadow-secondary/20 hover:shadow-secondary/40 hover:scale-[1.01] transition-all flex items-center justify-center gap-6">
                    <Save size={32} /> حفظ بيانات الترخيص
                 </button>
