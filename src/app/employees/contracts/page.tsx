...
import { FileText, Trash2, Plus, Calendar, Save, Building, Hash, User, Upload, Briefcase, Printer, Download, Edit } from "lucide-react";
  ...
  return (
    <div className="space-y-3 font-rubik">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <h1 className="text-lg font-black mb-2 tracking-tight text-white">عقود العمل</h1>
          <p className="text-sidebar-text font-medium text-sm">إدارة وتوثيق عقود الموظفين (قوى / ورقية) والاحتفاظ بنسخ رقمية.</p>
        </div>
      ...
      <div className="glass p-4 rounded-3xl border border-white/5 bg-white/[0.01]">
         <div className="grid grid-cols-1 lg:grid-cols-1 md:grid-cols-2 gap-5">
            {contracts.map((contract) => (
              <div key={contract.id} className="p-4 rounded-[3.5rem] bg-white/[0.02] border border-white/5 hover:border-secondary/20 transition-all group relative overflow-hidden flex flex-col md:flex-row items-center gap-5">
                 <div className="w-8 h-8 shrink-0 rounded-xl bg-emerald-500/10 flex items-center justify-center text-emerald-500 group-hover:scale-110 transition-transform">
                    <FileText size={40} />
                 </div>
                 <div className="flex-1 w-full space-y-3">
                    <div>
                       <h4 className="font-black text-lg text-white mb-1">{contract.employeeName || contract.employeeLink}</h4>
...
<button onClick={() => { if(window.confirm('هل أنت متأكد من الحذف؟ لا يمكن التراجع عن هذا الإجراء.')) removeItem(contract.id!); }} className=" p-3 text-rose-500 opacity-0 group-hover:opacity-100 transition-all hover:bg-rose-500/10 rounded-xl">
                    <Trash2 size={16} />
                 </button>
</div>
              </div>
         ...
         {contracts.length === 0 && !loading && (
           <div className="py-24 text-center border-2 border-dashed border-white/5 rounded-3xl">
              ...
              <p className="text-sidebar-text text-lg font-black">لا توجد عقود عمل مسجلة حالياً.</p>
           </div>
         )}
      </div>
        ...
        <div className="p-2">
           ...
           <div className="flex items-center justify-between mb-16 border-b border-white/5 pb-10">
              <div className="flex items-center gap-4 text-right">
                 ...
                 <div>
                    <h2 className="text-xl font-black text-white tracking-tighter">إضافة عقد عمل جديد</h2>
                    <p className="text-white/40 font-bold mt-3 text-lg">يرجى تعبئة كافة الحقول المطلوبة بدقة لمتابعة عقد العمل الموحد.</p>
                 </div>
              </div>
           </div>
           ...
           <form onSubmit={handleSubmit} className="space-y-3">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-x-4 gap-y-4">
                 ...
                 <div className="space-y-3 text-right">
                    <label className="text-sm font-black text-white/20 uppercase tracking-[0.2em] px-3">رقم العقد</label>
                    <div className="relative group">
                       <input 
                 ...
                 <div className="space-y-3 text-right">
                    <label className="text-sm font-black text-white/20 uppercase tracking-[0.2em] px-3">المهنة</label>
                    <div className="relative group">
                       <input 
                 ...
                 {/* Row 6 */}
                 <div className="space-y-3 text-right">
                    <label className="text-sm font-black text-white/20 uppercase tracking-[0.2em] px-3">الراتب</label>
                    <div className="relative group">
                       <input 
                 ...
                 <div className="space-y-3 text-right relative z-30">
                    <label className="text-sm font-black text-white/20 uppercase tracking-[0.2em] px-3">نوع التوظيف</label>
                    <CustomSelect 
                 ...
                 {/* Row 7 - Full Width */}
                 <div className="col-span-2 space-y-3 text-right">
                    <label className="text-sm font-black text-white/20 uppercase tracking-[0.2em] px-3">فترة الإشعار</label>
                    <div className="relative group">
                       <input 
                 ...
                 {/* Row 8 - Full Width */}
                 <div className="col-span-2 space-y-3 text-right">
                    <label className="text-sm font-black text-white/20 uppercase tracking-[0.2em] px-3">مرات التأخير</label>
                    <div className="relative group">
                       <input 
                 ...
                 <div className="col-span-2 space-y-3 text-right">
                    <label className="text-sm font-black text-white/20 uppercase tracking-[0.2em] px-3">ملف العقد (صورة أو PDF)</label>
