    ...
    <div className="space-y-3 font-rubik">
      ...
      <div className="relative glass p-14 rounded-3xl border border-secondary/10 bg-[#060b19] overflow-hidden flex flex-col items-center text-center">
         ...
         <div className="absolute top-4 left-10">
            <button className="flex items-center gap-3 px-3 py-2 rounded-3xl border border-white/10 bg-white/5 hover:bg-white/10 transition-all text-sm font-bold text-white/80">
         ...
         <div className="flex items-center justify-center gap-4 mb-6 z-10">
         ...
         <p className="text-lg text-sidebar-text font-bold max-w-2xl leading-relaxed z-10">
         ...
         <div className="grid grid-cols-1 md:grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div className="space-y-3">
               ...
               <div className="space-y-3 text-right">
                  ...
                  <div className="relative group">
                     <input 
                       ...
                       className="w-full bg-[#060b19] border border-white/10 rounded-xl py-2 px-3 text-sm outline-none focus:ring-4 focus:ring-secondary/10 focus:border-secondary/40 transition-all text-right placeholder:text-white/20" 
                     />
                     <Briefcase className="absolute left-8 top-1/2 -translate-y-1/2 text-white/10" size={18} />
                  </div>
               </div>
               ...
               <button 
                 ...
                 className="w-full bg-secondary text-primary py-6 rounded-2xl font-black text-lg shadow-2xl shadow-secondary/20 hover:shadow-secondary/40 hover:scale-[1.02] transition-all flex items-center justify-center gap-4 disabled:opacity-50 disabled:hover:scale-100"
               >
                 {isGenerating ? (
                   <>جاري التوليد بواسطة الذكاء الاصطناعي... <Loader2 size={18} className="animate-spin" /></>
                 ) : (
                   <>توليد المهام الوظيفية الذكية <Sparkles size={18} /></>
                 )}
               </button>
            </div>

            {/* Generated Results Area */}
            <div className="border border-white/5 bg-black/20 rounded-3xl p-4 flex flex-col justify-center min-h-[400px]">
               {isGenerating && (
                  <div className="text-center space-y-3">
                     <div className="w-8 h-8 border-4 border-secondary/20 border-t-secondary rounded-full animate-spin mx-auto"></div>
                     <div>
                        <h3 className="text-xl font-black text-white mb-2">الذكاء الاصطناعي يحلل...</h3>
               ...
               {!isGenerating && !generatedTasks && (
                  <div className="text-center space-y-3 opacity-30">
                     ...
                     <h3 className="text-xl font-black text-white">منطقة النتائج الذكية</h3>
                     <p className="text-sidebar-text">قم بإدخال بيانات الموظف والمسمى الوظيفي لتبدأ عملية التوليد.</p>
                  </div>
               )}

               {!isGenerating && generatedTasks && (
                  <div className="space-y-3 animate-in fade-in slide-in-from-bottom-10 duration-700">
                     <div className="flex items-center justify-between mb-2">
                        <h3 className="text-lg font-black text-secondary flex items-center gap-3">
                           <CheckCircle2 size={18} /> تم توليد المهام بنجاح
                        </h3>
                     ...
                     <button 
                       ...
                       className="w-full bg-emerald-500 text-white py-2 rounded-xl font-black text-sm shadow-xl shadow-emerald-500/20 hover:bg-emerald-400 transition-all flex items-center justify-center gap-3"
                     >
                        اعتماد وحفظ المهام للموظف <Save size={18} />
                     </button>
                  </div>
               )}
            </div>
      ...
      <div className="pt-10">
         <h3 className="text-xl font-black mb-8 flex items-center gap-3"><History className="text-secondary" /> سجل المهام المعينة</h3>
         <div className="grid grid-cols-1 md:grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {tasks.map(task => (
               <div key={task.id} className="glass p-4 rounded-xl border border-white/5 flex flex-col relative group">
                  {task.isAiGenerated && (
                  ...
                  <h4 className="font-bold text-sm mb-4 text-white pr-10">{task.title}</h4>
