...
import { Globe, Trash2, Plus, Calendar, Save, Building, Hash, User, Upload, Printer, Download, Edit } from "lucide-react";
  ...
  return (
    <div className="space-y-3 font-rubik">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <h1 className="text-lg font-black mb-2 tracking-tight text-white">جوازات السفر</h1>
          <p className="text-sidebar-text font-medium text-sm">إدارة جوازات سفر الموظفين ومتابعة تواريخ الانتهاء لتجنب الغرامات.</p>
        </div>
      ...
      <div className="glass p-4 rounded-3xl border border-white/5 bg-white/[0.01]">
         <div className="grid grid-cols-1 md:grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
            {passports.map((passport) => (
              <div key={passport.id} className="p-4 rounded-[3.5rem] bg-white/[0.02] border border-white/5 hover:border-secondary/20 transition-all group relative overflow-hidden">
                 <div className="flex items-center gap-4 mb-8">
                    <div className="w-16 h-12 rounded-[1.8rem] bg-indigo-500/10 flex items-center justify-center text-indigo-500 group-hover:scale-110 transition-transform">
                       <Globe size={16} />
                    </div>
                    <div>
                       <h4 className="font-black text-lg text-white">{passport.employeeName || passport.employeeLink}</h4>
...
<button onClick={() => { if(window.confirm('هل أنت متأكد من الحذف؟ لا يمكن التراجع عن هذا الإجراء.')) removeItem(passport.id!); }} className=" p-3 text-rose-500 opacity-0 group-hover:opacity-100 transition-all hover:bg-rose-500/10 rounded-xl">
                    <Trash2 size={16} />
                 </button>
</div>
              </div>
         ...
         {passports.length === 0 && !loading && (
           <div className="py-24 text-center border-2 border-dashed border-white/5 rounded-3xl">
              ...
              <p className="text-sidebar-text text-lg font-black">لا توجد جوازات سفر مسجلة حالياً.</p>
           </div>
         )}
      </div>
        ...
        <div className="p-2">
           <div className="flex items-center justify-between mb-16 border-b border-white/5 pb-10">
              <div className="flex items-center gap-4 text-right">
                 <div className="w-8 h-8 bg-indigo-500/10 rounded-2xl flex items-center justify-center text-indigo-500 border border-indigo-500/20 shadow-2xl">
                    <Globe size={18} />
                 </div>
                 <div>
                    <h2 className="text-xl font-black text-white tracking-tighter">إضافة جواز سفر جديد</h2>
                    <p className="text-white/40 font-bold mt-3 text-lg">يرجى تعبئة كافة الحقول المطلوبة بدقة لمتابعة صلاحية جواز السفر.</p>
                 </div>
              </div>
           </div>
           ...
           <form onSubmit={handleSubmit} className="space-y-3">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-x-4 gap-y-4">
                 ...
                 <div className="space-y-3 text-right">
                    <label className="text-sm font-black text-white/20 uppercase tracking-[0.2em] px-3">رقم الجواز</label>
                    <div className="relative group">
                       <input 
                 ...
                 <div className="col-span-2 space-y-3 text-right">
                    <label className="text-sm font-black text-white/20 uppercase tracking-[0.2em] px-3">صورة الجواز</label>
